...
void TCP::server(string server[]){
   ...
   while(1)
   {
      ...
      clnt_sock=accept(tcp->serv_sock, (struct sockaddr*)&clnt_adr,(socklen_t*)&clnt_adr_size);
      if(clnt_sock == -1){
...
void TCP::close_sock(){
   close(tcp->serv_sock);
   for(int i=0;i<number_of_sock;i++){
      close(tcp->clnt_socks[i]);
      close(tcp->new_sock[i]);
   }
   tcp->clnt_cnt =0;
